...
class DataforseoLabsGoogleKeywordsForCategoriesLiveRequestInfo(BaseModel):
    ...
    """ # noqa: E501
    category_codes: Optional[List[Optional[StrictStr]]] = Field(default=None, description=r"product and service categoriesrequired fieldThe maximum number of categories you can specify: 20you can download the full list of possible categories")
    ...
    language_code: Optional[StrictStr] = Field(default=None, description=r"unique language identifierrequired field if you don't specify language_nameNote: it is required to specify either language_name or language_codeyou can receive the list of available languages with their language_code by making a separate request to thehttps://api.dataforseo.com/v3/dataforseo_labs/locations_and_languagesexample:en")
    category_intersection: Optional[StrictBool] = Field(default=None, description=r"category intersectionsoptional fieldif set to true, you will get keywords featured in all specified categories;if set to false, you will keywords that are specified in any of the specified categories;default value: true")
