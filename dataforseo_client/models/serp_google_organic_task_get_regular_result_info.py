...
class SerpGoogleOrganicTaskGetRegularResultInfo(BaseModel):
    ...
    spell: Optional[SpellInfo] = Field(default=None, description=r"autocorrection of the search engine. if the search engine provided results for a keyword that was corrected, we will specify the keyword corrected by the search engine and the type of autocorrection;. if there is none, the value is null")
    refinement_chips: Optional[RefinementChipsInfo] = Field(default=None, description=r"search refinement chips. if there are none, the value is null")
    item_types: Optional[List[Optional[StrictStr]]] = Field(default=None, description=r"types of search results found in SERP. contains types of all search results (items) found in the returned SERP. possible item types:. answer_box, app, carousel, multi_carousel, featured_snippet, google_flights, google_reviews, third_party_reviews, google_posts, images, jobs, knowledge_graph, local_pack, hotels_pack, map, organic, paid, people_also_ask, related_searches, people_also_search, shopping, top_stories, twitter, video, events, mention_carousel, recipes, top_sights, scholarly_articles, popular_products, podcasts, questions_and_answers, find_results_on, stocks_box, visual_stories, commercial_units, local_services, google_hotels, math_solver, currency_box, product_considerations, found_on_web, short_videos, refine_products, explore_brands, perspectives, discussions_and_forums, compare_sites, courses, ai_overviewnote that this array contains all types of search results found in the returned SERP;. however, this endpoint provides data for featured_snippet, organic and paid types only;. to get all items (including SERP features and rich snippets) found in the returned SERP, please refer to the Google Organiс Advanced SERP endpoint")
    ...
    items: Optional[List[Optional[BaseSerpApiElementItem]]] = Field(default=None, description=r"items in SERP")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        _dict['pages_count'] = self.pages_count
