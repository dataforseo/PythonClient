...
class BacklinksHistoryLiveItem(BaseModel):
    ...
    rank: Optional[StrictInt] = Field(default=None, description=r"*domain rank on the given `date`*. learn more about the metric and how it is calculated in [this help center article](https://dataforseo.com/help-center/what_is_rank_in_backlinks_api)")
    backlinks: Optional[StrictInt] = Field(default=None, description=r"*number of backlinks*")
    new_backlinks: Optional[StrictInt] = Field(default=None, description=r"*number of new backlinks for the `target`*. data is provided based in a comparison with the previous period. **Note:** this data is available from May 2021;. if the date range specified in the POST request precedes May 2021, the field will equal `0`")
    lost_backlinks: Optional[StrictInt] = Field(default=None, description=r"*number of lost backlinks for the `target`*. data is provided based in a comparison with the previous period. **Note:** this data is available from May 2021;. if the date range specified in the POST request precedes May 2021, the field will equal `0`")
    new_referring_domains: Optional[StrictInt] = Field(default=None, description=r"*number of new referring domains for the `target`*. data is provided based in a comparison with the previous period. **Note:** this data is available from May 2021;. if the date range specified in the POST request precedes May 2021, the field will equal `0`")
    lost_referring_domains: Optional[StrictInt] = Field(default=None, description=r"*number of lost referring domains for the `target`*. data is provided based in a comparison with the previous period. **Note:** this data is available from May 2021;. if the date range specified in the POST request precedes May 2021, the field will equal `0`")
    ...
    broken_pages: Optional[StrictInt] = Field(default=None, description=r"*number of broken pages*. number of pages that receive backlinks but respond with 4xx or 5xx status codes")
    ...
    referring_pages: Optional[StrictInt] = Field(default=None, description=r"*number of pages pointing to the `target`*")
    ...
    referring_links_attributes: Optional[Dict[str, Optional[StrictInt]]] = Field(default=None, description=r"*link attributes of the referring links*. indicates link attributes of the referring links and link count per each attribute")
