...
class OnPageHtmlResourceItem(BaseOnPageResourceItem):
    ...
    resource_errors: Optional[OnPageResourceIssueInfo] = Field(default=None, description=r"*resource errors and warnings*")
    size: Optional[StrictInt] = Field(default=None, description=r"*resource size*. indicates the size of a given page measured in bytes")
    encoded_size: Optional[StrictInt] = Field(default=None, description=r"*page size after encoding*. indicates the size of the encoded page measured in bytes")
    total_transfer_size: Optional[StrictInt] = Field(default=None, description=r"*compressed page size*. indicates the compressed size of a given page")
    fetch_time: Optional[StrictStr] = Field(default=None, description=r"*date and time when a resource was fetched*. in the UTC format: “yyyy-mm-dd hh-mm-ss +00:00”. example:. `2019-11-15 12:57:46 +00:00`")
    cache_control: Optional[CacheControl] = Field(default=None, description=r"*instructions for caching*")
    checks: Optional[Dict[str, Optional[StrictBool]]] = Field(default=None, description=r"*website checks*. on-page check-ups related to the page")
    content_encoding: Optional[StrictStr] = Field(default=None, description=r"*type of encoding*")
    media_type: Optional[StrictStr] = Field(default=None, description=r"*types of media used to display a page*")
    server: Optional[StrictStr] = Field(default=None, description=r"*server version*")
    last_modified: Optional[LastModified] = Field(default=None, description=r"*contains data on changes related to the resource*. if there is no data, the value will be `null`")
    meta: Optional[PageMetaInfo] = Field(default=None, description=r"*page properties*. the value depends on the `resource_type`")
    page_timing: Optional[PageTiming] = Field(default=None, description=r"*object of page load metrics*")
    onpage_score: Optional[StrictFloat] = Field(default=None, description=r"*shows how page is optimized on a 100-point scale*. this field shows how page is optimized considering critical on-page issues and warnings detected;. `100` is the highest possible score that means the page does not have any critical on-page issues and important warnings;. learn more about how the metric is calculated in [this help center article](https://dataforseo.com/help-center/how-on-page-seo-score-is-calculated)")
    total_dom_size: Optional[StrictInt] = Field(default=None, description=r"*total [DOM](https://developers.google.com/web/tools/chrome-devtools/dom) size of a page*")
    custom_js_response: Optional[Any] = Field(default=None, description=r"*the result of executing a specified JS script*. **note** that you should specify a `custom_js` field when [setting a task](/v3/on_page/task_post/) to receive this data and the field type and its value will totally depend on the script you specified;. you can also filter the results by this value specifying `filters` in the following way:. `['custom_js_response.url', 'like', 'pixel']`")
    custom_js_client_exception: Optional[StrictStr] = Field(default=None, description=r"*error when executing a custom js*. if the error occurred when executing the script you specified in the `custom_js` field, the error message would be displayed here")
    broken_resources: Optional[StrictBool] = Field(default=None, description=r"*indicates whether a page contains broken resources*")
    broken_links: Optional[StrictBool] = Field(default=None, description=r"*indicates whether a page contains broken links*")
    duplicate_title: Optional[StrictBool] = Field(default=None, description=r"*indicates whether a page has duplicate `title` tags*")
    duplicate_description: Optional[StrictBool] = Field(default=None, description=r"*indicates whether a page has a duplicate description*")
    duplicate_content: Optional[StrictBool] = Field(default=None, description=r"*indicates whether a page has duplicate content*")
    click_depth: Optional[StrictInt] = Field(default=None, description=r"*number of clicks it takes to get to the page*. indicates the number of clicks from the homepage needed before landing at the target page")
    is_resource: Optional[StrictBool] = Field(default=None, description=r"*indicates whether a page is a single resource*")
    url_length: Optional[StrictInt] = Field(default=None, description=r"*page URL length in characters*")
    relative_url_length: Optional[StrictInt] = Field(default=None, description=r"*relative URL length in characters*")
