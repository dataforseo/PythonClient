...
class SerpApiKnowledgeGraphAiOverviewItemElementItem(BaseSerpApiKnowledgeGraphElementItem):
    ...
    asynchronous_ai_overview: Optional[StrictBool] = Field(default=None, description=r"indicates whether the element is loaded asynchronously. if true, the ai_overview element is loaded asynchronously;. if false, the ai_overview element is loaded from cache;. to obtain the content of ai_overview elements, use the load_async_ai_overview parameter in the POST request")
    items: Optional[List[Optional[BaseSerpApiAiOverviewElementItem]]] = Field(default=None, description=r"popular keywords relevant to the initial search query. if there are none, equals null")
