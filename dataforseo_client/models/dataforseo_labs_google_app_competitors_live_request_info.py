...
class DataforseoLabsGoogleAppCompetitorsLiveRequestInfo(BaseModel):
    ...
    order_by: Optional[List[Optional[StrictStr]]] = Field(default=None, description="results sorting rules. optional field. you can use the same values as in the filters array to sort the results;. possible sorting types:. asc – results will be sorted in the ascending order;. desc – results will be sorted in the descending order;. you should use a comma to specify a sorting type;. example:. ['intersections,asc']. Note: you can set no more than three sorting rules in a single request;. you should use a comma to separate several sorting rules;. example:. ['intersections,desc','sum_position,asc']. default rule:. ['intersections,desc']. Note: if the item_types array contains item types that are different from organic, the results will be ordered by the first item type in the array")
