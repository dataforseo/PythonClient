...
class KeywordsDataGoogleAdsSearchVolumeLiveResultInfo(BaseModel):
    ...
    spell: Optional[StrictStr] = Field(default=None, description=r"correct spelling of the keyword. Note:if the keyword in the POST array appears to be misspelled, data will be returned for the correctly spelled keyword;. we use the functionality of Google Ads API to check and validate the spelling of keywords, learn more by this link")
    ...
    competition: Optional[StrictStr] = Field(default=None, description=r"competition. represents the relative amount of competition associated with the given keyword in paid SERP only;. this value is based on Google Ads data and can take the following values: HIGH, MEDIUM, LOW;. if there is no data the value is null;. learn more about the metric in this help center article")
    competition_index: Optional[StrictInt] = Field(default=None, description=r"competition. represents the relative amount of competition associated with the given keyword in paid SERP only;. this value is based on Google Ads data and can be between 0 and 100 (inclusive);. if there is no data the value is null;. learn more about the metric in this help center article")
    search_volume: Optional[StrictInt] = Field(default=None, description=r"monthly average search volume rate;. represents either the (approximate) number of searches for the given keyword idea on google.com or google.com and partners, depending on the user’s targeting;. if there is no data then the value is null")
    ...
    high_top_of_page_bid: Optional[StrictInt] = Field(default=None, description=r"maximum bid for the ad to be displayed at the top of the first page. indicates the value greater than about 80% of the lowest bids for which ads were displayed (based on Google Ads statistics for advertisers);. the value may differ depending on the location specified in a POST request")
