...
class OnPagePagesByResourceRequestInfo(BaseModel):
    ...
    url: Optional[StrictStr] = Field(default=None, description=r"*resource URL*. **required field**. you can get this URL in the response of the [Resources](/v3/on_page/resources/) endpoint. example:. `https://ajax.googleapis.com/ajax/libs/jquery/1.12.4/jquery.min.js`")
    ...
    order_by: Optional[List[Optional[StrictStr]]] = Field(default=None, description=r"*results sorting rules*. optional field. you can use the same values as in the `filters` array to sort the results. possible sorting types:. `asc` - results will be sorted in the ascending order. `desc` - results will be sorted in the descending order. you should use a comma to set up a sorting type. example:. `['meta.external_links_count,desc']`. **note that you can set no more than three sorting rules in a single request**. you should use a comma to separate several sorting rules. example:. `['page_timing.dom_complete,asc','size,desc']`")
