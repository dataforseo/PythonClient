...
class SerpGoogleMapsTaskPostRequestInfo(BaseModel):
    ...
    depth: Optional[StrictInt] = Field(default=None, description=r"*parsing depth*. optional field. number of results in SERP. default value: `100`. max value: `700`. **Your account will be billed per each SERP containing up to 100 results;**. Setting depth above 100 may result in additional charges [if the search engine returns more than 100 results](https://dataforseo.com/help-center/how-many-results-scraped);. The cost can be calculated on the [Pricing](https://dataforseo.com/pricing/serp/google-maps-serp-api) page.")
    priority: Optional[StrictInt] = Field(default=None, description=r"*task priority*. optional field. can take the following values:. 1 – normal execution priority (set by default). 2 – high execution priority. You will be additionally charged for the tasks with high execution priority.. The cost can be calculated on the [Pricing](https://dataforseo.com/pricing/serp/google-maps-serp-api) page.")
    device: Optional[StrictStr] = Field(default=None, description=r"*device type*. optional field. return results for a specific device type. can take the values:`desktop`, `mobile`. default value: `desktop`. **note:** for `mobile` device, only 20 results are returned for every SERP")
    ...
    se_domain: Optional[StrictStr] = Field(default=None, description=r"*search engine domain*. optional field. we choose the relevant search engine domain automatically according to the location and language you specify. however, you can set a custom search engine domain in this field. example:. `google.co.uk`")
    search_this_area: Optional[StrictBool] = Field(default=None, description=r"*show results from the displayed area*. optional field. can take the values:`true`, `false`. default value: `true`. if set to `false`, the `search_this_area` mode will be turned off. **Note:** if the `search_this_area` mode is turned off, Google Maps listings might contain results beyond the displayed area")
    search_places: Optional[StrictBool] = Field(default=None, description=r"*search places mode*. optional field. the search places mode allows to obtain Google Maps results on a certain place (e.g., Apple Store in New York). however, due to the pecularities of our data mining algorithm, this mode might interfere with some local-intent queries - and display results for a location that is different from that specified in the request;. to prevent this interference and obtain correct results for keywords with local intent you may set this parameter to `false`;default value: `true`. **Note:** if the `search_places` mode is turned off and no results were found in the search area, the `results` array will be empty")
