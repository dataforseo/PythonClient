...
class SerpGoogleAdsSearchTaskPostRequestInfo(BaseModel):
    ...
    """ # noqa: E501
    advertiser_ids: Optional[List[Optional[StrictStr]]] = Field(default=None, description=r"*advertiser identifiers*. **required field if `target` is not specified**. you can specify the maximum of 25 values in this array;. `advertiser_ids` values for this parameter can be found in the [Google Ads Advertisers](http://docs.dataforseo.com/v3/serp/google/ads_advertisers/task_get/advanced/) endpoint;")
    target: Optional[StrictStr] = Field(default=None, description=r"*domain name*. **required field if `advertiser_ids` is not specified**. domain name associated with an advertiser account")
    ...
    postback_data: Optional[StrictStr] = Field(default=None, description=r"*postback_url datatype*. **required field if you specify `postback_url`**. corresponds to the function you used for setting a task. possible values:. `advanced`")
    ...
    location_coordinate: Optional[StrictStr] = Field(default=None, description=r"*GPS coordinates of a location*. optional field. **if you use this field, you don't need to specify `location_name` or `location_code`**. example:. `52.6178549,-155.352142`. **Note:** if you don't specify `location_name`, `location_code`, or `location_coordinate`, the ads will be searched across all the available locations")
    ...
    platform: Optional[StrictStr] = Field(default=None, description=r"*advertising platform*. optional field. possible values: `all`, `google_play`, `google_maps`, `google_search`, `google_shopping`, `youtube`. default value: `all`")
    format: Optional[StrictStr] = Field(default=None, description=r"*ad format*. optional field. possible values: `all`, `text`, `image`, `video`")
    date_from: Optional[StrictStr] = Field(default=None, description=r"*starting date of the time range*. optional field. required field if `date_to` is specified; . date format: `'yyyy-mm-dd'`. minimum value: 2018-05-31. maximum value: today's date. example:. `'2020-01-01'`")
    date_to: Optional[StrictStr] = Field(default=None, description=r"*ending date of the time range*. optional field. required field if `date_from` is specified; . date format: `'yyyy-mm-dd'`. minimum value: 2018-05-31. maximum value: today's date. example:. `'2020-01-01'`")
