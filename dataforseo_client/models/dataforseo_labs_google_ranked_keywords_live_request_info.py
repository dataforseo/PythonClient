...
class DataforseoLabsGoogleRankedKeywordsLiveRequestInfo(BaseModel):
    ...
    """ # noqa: E501
    target: Optional[StrictStr] = Field(default=None, description=r"domain name or page url. required field. the domain name of the target website, subdomain or URL of the target webpage;. the domain name must be specified without https:// or www.;. the subdomain must be specified without https://;. the webpage URL must be specified with https:// or www.. Note: if you specify the webpage URL without https:// or www., the result will be returned for the entire domain rather than the specific page")
    ...
    load_rank_absolute: Optional[StrictBool] = Field(default=None, description=r"return rankings distribution by rank_absolute. optional field. default value: false. if set to true, we will return the field metrics_absolute containing rankings distribution by the rank_absolute parameter that indicates the result’s position among all SERP elements")
    historical_serp_mode: Optional[StrictStr] = Field(default=None, description=r"data collection mode. optional field. you can use this field to filter the results;. possible types of filtering:. live — return keywords for which the specified target currently has ranking results in SERP;. lost — return keywords for which the specified target had previously had ranking results in SERP, but didn’t have them during the last check;. all — return both types of keywords.. default value: live")
    ...
    order_by: Optional[List[Optional[StrictStr]]] = Field(default=None, description=r"results sorting rules. optional field. you can use the same values as in the filters array to sort the results. possible sorting types:. asc – results will be sorted in the ascending order. desc – results will be sorted in the descending order. you should use a comma to set up a sorting type. example:. ['keyword_data.keyword_info.competition,desc']. default rule:. ['ranked_serp_element.serp_item.rank_group,asc']. note that you can set no more than three sorting rules in a single request. you should use a comma to separate several sorting rules. example:. ['keyword_data.keyword_info.search_volume,desc','keyword_data.keyword_info.cpc,desc']")
