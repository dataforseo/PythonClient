...
class DomainInfo(BaseModel):
    ...
    DomainInfo
    """ # noqa: E501
    name: Optional[StrictStr] = Field(default=None, description=r"*domain name*")
    cms: Optional[StrictStr] = Field(default=None, description=r"*content management system*. content management system identified on a website. the content of_the `generator`_meta tag. the data is taken from the first random page that returns the 200 response code. if our crawler was unable to identify the cms, the value would be `null`n")
    ip: Optional[StrictStr] = Field(default=None, description=r"*domain ip address*")
    server: Optional[StrictStr] = Field(default=None, description=r"*website server*. the version of the server detected on a website. the content of the `server` header. the information is taken from the first page which response code is 200")
    crawl_start: Optional[StrictStr] = Field(default=None, description=r"*time when the crawling start*. date and time when the website was sent for crawling. in the UTC format: “yyyy-mm-dd hh-mm-ss +00:00”. example:. `2019-11-15 12:57:46 +00:00`")
    crawl_end: Optional[StrictStr] = Field(default=None, description=r"*time when the crawling ended*. date and time when the crawling was finished. in the UTC format: “yyyy-mm-dd hh-mm-ss +00:00”. example:. `2019-11-15 12:57:46 +00:00`. **Note:** informative only if `'crawl_progress'` is `'finished'`. if `'crawl_progress'` is `in_progress`, the value will be `null`")
    extended_crawl_status: Optional[StrictStr] = Field(default=None, description=r"*crawl status and errors*. indicates the reason why a website was not crawled;. can take the following values:. `no_errors` - no crawling errors were detected;. `site_unreachable` - our crawler could not reach a website and thus was not able to obtain a status code;. `invalid_page_status_code` - status code of the first crawled page >= 400;. `forbidden_meta_tag` - the first crawled page contains the  tag;. `forbidden_robots` - robots.txt forbids crawling the page;. `forbidden_http_header` - HTTP header of the page contains 'X-Robots-Tag: noindex' ;. `too_many_redirects` - the first crawled page has more than 10 redirects;. `unknown` - the reason is unknown")
    ...
    www_redirect_status_code: Optional[StrictInt] = Field(default=None, description=r"*redirect status code*. the status code of the www to non-www redirect. in most cases, it is recommended that redirect returns a `301` status code")
    main_domain: Optional[StrictStr] = Field(default=None, description=r"*root domain name*")
