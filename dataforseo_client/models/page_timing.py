...
class PageTiming(BaseModel):
    ...
    largest_contentful_paint: Optional[StrictFloat] = Field(default=None, description=r"*Core Web Vitals metric measuring how fast the largest above-the-fold content element is displayed*. The amount of time (in milliseconds) to render the largest content element visible in the viewport, from when the user requests the URL. [Learn more](https://web.dev/lcp/).")
    first_input_delay: Optional[StrictFloat] = Field(default=None, description=r"*Core Web Vitals metric indicating the responsiveness of a page*. The time (in milliseconds) from when a user first interacts with your page to the time when the browser responds to that interaction. [Learn more](https://web.dev/fid/).")
