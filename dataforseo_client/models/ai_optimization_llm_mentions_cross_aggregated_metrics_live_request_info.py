...
class AiOptimizationLlmMentionsCrossAggregatedMetricsLiveRequestInfo(BaseModel):
    ...
    aggregation_key: Optional[StrictStr] = Field(default=None, description=r"aggregation key for grouping the results. required field. groups results for comparison and serves as a label for the group;. you can specify up to 250 characters in the aggregation_key field")
    ...
    domain: Optional[StrictStr] = Field(default=None, description=r"target domain. required field if you don’t specify a keyword. you can specify up to 63 characters in the domain field;. a domain should be specified without https:// and www.")
