...
class MerchantAmazonSellersLiveAdvancedRequestInfo(BaseModel):
    """
    ...
    se_domain: Optional[StrictStr] = Field(default=None, description=r"*search engine domain*. optional field. we choose the relevant search engine domain automatically according to the location and language you specify. however, you can set a custom search engine domain in this field. example:. *`amazon.co.uk`*, *`amazon.com.au`*, *`amazon.de`*, etc.")
