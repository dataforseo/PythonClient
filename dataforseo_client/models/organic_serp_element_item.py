...
class OrganicSerpElementItem(BaseSerpApiElementItem):
    ...
    related_search_url: Optional[StrictStr] = Field(default=None, description=r"*URL to a similar search*. URL to a new search for the same keyword(s) [on related sites](https://support.google.com/websearch/answer/2466433?hl=en#:~:text=Search%20for%20related%20sites)")
    website_name: Optional[StrictStr] = Field(default=None, description=r"*name of the website in SERP*")
    is_image: Optional[StrictBool] = Field(default=None, description=r"*indicates whether the element contains an_`image`*. **Note:** this check no longer appears in SERPn")
    ...
    faq: Optional[FaqBox] = Field(default=None, description=r"*frequently asked questions*. questions and answers extension shown below some of Google's search results. **Note:** this object is deprecated and always returns `null`", deprecated=True)
    extended_people_also_search: Optional[List[Optional[StrictStr]]] = Field(default=None, description=r"*extension of the organic element*. extension of the organic result containing related search queries. **Note:** extension appears in SERP upon clicking on the result and then bouncing back to search results")
    ...
    related_result: Optional[List[Optional[RelatedResult]]] = Field(default=None, description=r"*related result from the same domain*. related result from the same domain appears as a part of the main result snippet;. you can derive the `related_result` snippets as `'type': 'organic'` results by setting the `group_organic_results` parameter to `false` in the POST request")
