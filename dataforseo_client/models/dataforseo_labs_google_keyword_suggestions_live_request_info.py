...
class DataforseoLabsGoogleKeywordSuggestionsLiveRequestInfo(BaseModel):
    ...
    order_by: Optional[List[Optional[StrictStr]]] = Field(default=None, description="results sorting rules. optional field. you can use the same values as in the filters array to sort the results. possible sorting types:. asc – results will be sorted in the ascending order. desc – results will be sorted in the descending order. a comma is used as a separator. example:. ['keyword_info.competition,desc']. default rule:. ['keyword_info.search_volume,desc']. note that you can set no more than three sorting rules in a single request. you should use a comma to separate several sorting rules. example:. ['keyword_info.search_volume,desc','keyword_info.cpc,desc']")
