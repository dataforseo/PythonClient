...
class KeywordsDataBingKeywordsForKeywordsLiveResultInfo(BaseModel):
    ...
    cpc: Optional[StrictFloat] = Field(default=None, description=r"*cost-per-click*. represents the average cost per click (USD) historically paid for the keyword.. if there is no data, then the value is_`null`n")
    search_volume: Optional[StrictInt] = Field(default=None, description=r"*monthly average search volume rate*. represents the (approximate) number of searches for the keyword on the Bing search engine, depending on the user’s targetingsearch volume is rounded to the closest decimal values. if there is no data, then the value is_`null`n")
