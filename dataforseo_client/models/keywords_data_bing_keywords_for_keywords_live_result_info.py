...
class KeywordsDataBingKeywordsForKeywordsLiveResultInfo(BaseModel):
    ...
    search_partners: Optional[StrictBool] = Field(default=None, description=r"indicates whether data from partner networks is included in the response")
    device: Optional[StrictStr] = Field(default=None, description=r"device type. indicates for what device type the data is provided;. possible values: all, mobile, desktop, tablet")
    competition: Optional[StrictInt] = Field(default=None, description=r"competition. represents the relative amount of competition associated with the given keyword in paid SERP only. This value is based on Bing Ads data.. Possible values: 0.1, 0.5,0.90.1 – low competition,. 0.5 – medium competition,. 0.9 – high competition;. if there is no data the value is null")
    cpc: Optional[StrictFloat] = Field(default=None, description=r"cost-per-click. represents the average cost per click (USD) historically paid for the keyword.. if there is no data, then the value is null")
    search_volume: Optional[StrictInt] = Field(default=None, description=r"monthly average search volume rate. represents the (approximate) number of searches for the keyword on the Bing search engine, depending on the user’s targetingsearch volume is rounded to the closest decimal values. if there is no data, then the value is null")
