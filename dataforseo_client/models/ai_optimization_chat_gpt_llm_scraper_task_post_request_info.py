...
class AiOptimizationChatGptLlmScraperTaskPostRequestInfo(BaseModel):
    ...
    force_web_search: Optional[StrictBool] = Field(default=None, description=r"force AI agent to use web searchoptional fieldwhen enabled, the AI model is forced to access and cite current web information;default value: false;Note: even if the parameter is set to true, there is no guarantee web sources will be cited in the response")
    expand_citations: Optional[StrictBool] = Field(default=None, description=r"return expanded citation bar in HTML resultsoptional fieldto enable this parameter, force_web_search must also be enabled;when enabled, the HTML endpoint will return data from the expanded citation bar;default value: false")
