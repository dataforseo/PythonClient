...
class KeywordsDataGoogleAdsKeywordsForSiteLiveResultInfo(BaseModel):
    ...
    spell: Optional[StrictStr] = Field(default=None, description=r"")
    location_code: Optional[StrictInt] = Field(default=None, description=r"*location code in a POST array*. if there is no data, the value is_`null`n")
    language_code: Optional[StrictStr] = Field(default=None, description=r"*language code in a POST array*. if there is no data, the value is_`null`n")
    search_partners: Optional[StrictBool] = Field(default=None, description=r"*include Google search partners*. the value you specified when setting the task. if `true`, the results are returned for owned, operated, and syndicated networks across Google and partner sites that host Google search;. if `false`, the results are returned for Google search sites only")
    competition: Optional[StrictStr] = Field(default=None, description=r"*competition*. represents the relative level of competition associated with the given keyword in paid SERP only. possible values: `LOW`, `MEDIUM`, `HIGH`. if competition level is unknown, the value is `null`;. learn more about the metric in [this help center article](https://dataforseo.com/help-center/what-is-competition)")
    competition_index: Optional[StrictInt] = Field(default=None, description=r"*competition index*. the competition index for the query indicating how competitive ad placement is for the keyword. can take values from 0 to 100. the level of competition from 0 to 100 is determined by the number of ad slots filled divided by the total number of ad slots available. if not enough data is available, the value is `null`;. learn more about the metric in [this help center article](https://dataforseo.com/help-center/what-is-competition)")
    search_volume: Optional[StrictInt] = Field(default=None, description=r"*monthly average search volume rate*. represents the (approximate) number of searches for the given keyword idea either on google.com or google.com and partners, depending on the user’s targeting. if there is no data, the value is `null`")
    ...
    cpc: Optional[StrictFloat] = Field(default=None, description=r"*cost per click*. indicates the amount paid (USD) for each click on the ad displayed for a given keyword")
    monthly_searches: Optional[List[Optional[MonthlySearchesInfo]]] = Field(default=None, description=r"*monthly searches*. represents the (approximate) number of searches on this keyword idea (as available for the past twelve months), targeted to the specified geographic locations. if there is no data, the value is `null`")
