...
class OnPageLighthouseTaskPostRequestInfo(BaseModel):
    ...
    categories: Optional[List[Optional[StrictStr]]] = Field(default=None, description=r"categories of Lighthouse audits. optional field. each category is a collection of audits and audit groups that applies weighting and scoring to the section (see official definition)if you ignore this field, we will return data for all categories unless you specify audits. use this field to get data for specific categories you indicate herepossible values:. seo, performance, best_practices, accessibility")
