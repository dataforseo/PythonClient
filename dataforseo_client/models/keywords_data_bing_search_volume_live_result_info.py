...
class KeywordsDataBingSearchVolumeLiveResultInfo(BaseModel):
    ...
    search_volume: Optional[StrictInt] = Field(default=None, description=r"*monthly average search volume rate*represents either the (approximate) number of searches for the given keyword idea on bing search engine depending on the user’s targeting;. search volume is rounded to the nearest tens;. if there is no data, the value is_`null`n")
    categories: Optional[List[Optional[StrictStr]]] = Field(default=None, description=r"*product and service categories*. our API doesn't return categories for this endpoint: the parameter will always equal `null`")
