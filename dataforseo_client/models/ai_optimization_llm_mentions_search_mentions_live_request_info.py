...
class AiOptimizationLlmMentionsSearchMentionsLiveRequestInfo(BaseModel):
    """
    ...
    search_after_token: Optional[StrictStr] = Field(default=None, description=r"*token for subsequent requests*. optional field. provided in the identical filed of the response to each request;. use this parameter to avoid timeouts while trying to obtain over `1000` results in a single request;. by specifying the unique `search_after_token` value from the response array, you will get the subsequent results of the initial task;. `search_after_token` values are unique for each subsequent task ;. **Note:** if the `search_after_token` is specified in the request, all other parameters should be identical to the previous request")
