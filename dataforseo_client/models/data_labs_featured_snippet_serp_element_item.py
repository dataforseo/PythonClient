...
class DataLabsFeaturedSnippetSerpElementItem(BaseDataforseoLabsApiElementItem):
    ...
    etv: Optional[StrictFloat] = Field(default=None, description=r"*estimated traffic volume*.             estimated organic monthly traffic a featured URL delivers to the domain.             calculated as the product of CTR (click-through-rate) and search volume values of the returned keyword.             learn more about how the metric is calculated in [this help center article](https://dataforseo.com/help-center/how-is-etv-calculated)")
    estimated_paid_traffic_cost: Optional[StrictFloat] = Field(default=None, description=r"*estimated cost of converting organic search traffic into paid*.             represents the estimated monthly cost of running ads for the returned keyword.             the metric is calculated as the product of organic `etv` and paid `cpc` values and indicates the cost of driving the estimated volume of monthly organic traffic through PPC advertising in Google Search.             learn more about how the metric is calculated in [this help center article](https://dataforseo.com/help-center/how-is-traffic-cost-calculated)")
    clickstream_etv: Optional[StrictFloat] = Field(default=None, description=r"")
    rank_changes: Optional[RankChanges] = Field(default=None, description=r"*changes in rankings*.             ranking changes of the SERP element compared to the preceding month;.             **Note:** the changes are calculated even if the preceding month is not included in a POST request")
