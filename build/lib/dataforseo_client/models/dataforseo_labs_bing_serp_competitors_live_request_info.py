...
class DataforseoLabsBingSerpCompetitorsLiveRequestInfo(BaseModel):
    ...
    order_by: Optional[List[Optional[StrictStr]]] = Field(default=None, description=r"results sorting rules. optional field. you can use the same values as in the filters array to sort the results. possible sorting types:. asc – results will be sorted in the ascending order. desc – results will be sorted in the descending order. the comma is used as a separator. example:. ['avg_position,asc']. default rule:. ['rating,desc']. note that you can set no more than three sorting rules in a single request. you should use a comma to separate several sorting rules. example:. ['avg_position,asc','etv,desc']")
