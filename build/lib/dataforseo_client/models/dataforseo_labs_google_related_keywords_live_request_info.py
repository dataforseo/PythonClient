...
class DataforseoLabsGoogleRelatedKeywordsLiveRequestInfo(BaseModel):
    ...
    depth: Optional[StrictInt] = Field(default=None, description=r"keyword search depth. optional field. default value: 1. number of the returned results depends on the value you set in this field. you can specify a level from 0 to 4. estimated number of keywords for each level (maximum):. 0 – the keyword set in the keyword field. 1 – 8 keywords. 2 – 72 keywords. 3 – 584 keywords. 4 – 4680 keywords")
    ...
    order_by: Optional[List[Optional[StrictStr]]] = Field(default=None, description=r"results sorting rules. optional field. you can use the same values as in the filters array to sort the results. possible sorting types:. asc – results will be sorted in the ascending order. desc – results will be sorted in the descending order. you should use a comma to set up a sorting type. example:. ['keyword_data.keyword_info.competition,desc']. default rule:. ['keyword_data.keyword_info.search_volume,desc']. note that you can set no more than three sorting rules in a single request. you should use a comma to separate several sorting rules. example:. ['keyword_data.keyword_info.search_volume,desc','keyword_data.keyword_info.cpc,desc']")
