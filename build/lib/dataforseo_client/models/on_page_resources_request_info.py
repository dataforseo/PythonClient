...
class OnPageResourcesRequestInfo(BaseModel):
    ...
    url: Optional[StrictStr] = Field(default=None, description=r"page URL. optional field. specify this field if you want to get the resources for a specific page. note that to obtain resource’s meta from a particular URL, you should specify the URL in this field;. if you do not indicate a url when setting a task, resource’s meta in the results will be returned based on the data from the page where our crawler first saw the resource")
    limit: Optional[StrictInt] = Field(default=None, description=r"the maximum number of returned resources. optional field. default value: 100. maximum value: 1000")
    offset: Optional[StrictInt] = Field(default=None, description=r"offset in the results array of returned resources. optional field. default value: 0. if you specify the 10 value, the first ten resources in the results array will be omitted and the data will be provided for the successive resources")
    ...
    relevant_pages_filters: Optional[List[Optional[StrictStr]]] = Field(default=None, description=r"filter the resources by relevant pages. optional field. you can use this field to obtain resources from pages matching to the defined parameters. you can apply the same filters here as available for the pages endpoint. you can add several filters at once (8 filters maximum). you should set a logical operator and, or between the conditions. the following operators are supported:. regex, not_regex, <, <=, >, >=, =, <>, in, not_in, like, not_like. you can use the % operator with like and not_like to match any string of zero or more characters. example:. ['checks.no_image_title','=',true]")
    order_by: Optional[List[Optional[StrictStr]]] = Field(default=None, description=r"results sorting rules. optional field. you can use the same values as in the filters array to sort the results. possible sorting types:. asc – results will be sorted in the ascending order. desc – results will be sorted in the descending order. you should use a comma to set up a sorting type. example:. ['size,desc']. note that you can set no more than three sorting rules in a single request. you should use a comma to separate several sorting rules. example:. ['size,desc','fetch_timing.fetch_end,desc']")
