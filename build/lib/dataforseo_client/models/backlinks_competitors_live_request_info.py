...
class BacklinksCompetitorsLiveRequestInfo(BaseModel):
    ...
    """ # noqa: E501
    target: Optional[StrictStr] = Field(default=None, description=r"domain, subdomain or webpage to get competitor domains for. required field. a domain or a subdomain should be specified without https:// and www.. a page should be specified with absolute URL (including http:// or https://)")
    ...
    order_by: Optional[List[Optional[StrictStr]]] = Field(default=None, description=r"results sorting rules. optional field. you can use the same values as in the filters array to sort the results. possible sorting types:. asc – results will be sorted in the ascending order. desc – results will be sorted in the descending order. you should use a comma to set up a sorting type. example:. ['rank,desc']. note that you can set no more than three sorting rules in a single request. you should use a comma to separate several sorting rules. example:. ['intersections,desc','rank,asc']")
    main_domain: Optional[StrictBool] = Field(default=None, description=r"indicates if only main domain of the target will be included in the search. optional field. if set to true, only the main domain will be included in search;. default value: true")
    exclude_large_domains: Optional[StrictBool] = Field(default=None, description=r"indicates whether large domain will appear in results. optional field. if set to true, the results from the large domain (google.com, amazon.com, etc.) will be omitted;. default value: true")
