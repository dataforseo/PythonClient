...
class SerpGoogleMapsLiveAdvancedResultInfo(BaseModel):
    ...
    check_url: Optional[StrictStr] = Field(default=None, description=r"direct URL to search engine results. you can use it to make sure that we provided exact results. Note: to check location-specific results, follow the provided check url, scroll up and down, then click the “Search this area” button")
    ...
    item_types: Optional[List[Optional[StrictStr]]] = Field(default=None, description=r"types of search results in SERP. contains types of search results (items) found in SERP.. possible item types:. maps_search, maps_paid_item")
    ...
    items: Optional[List[Optional[BaseSerpApiGoogleMapsElementItem]]] = Field(default=None, description=r"items of the element")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "items": [BaseSerpApiGoogleMapsElementItem.from_dict(_item) for _item in obj["items"]] if obj.get("items") is not None else None,
        })
