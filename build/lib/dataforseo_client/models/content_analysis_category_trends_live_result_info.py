...
class ContentAnalysisCategoryTrendsLiveResultInfo(BaseModel):
    ...
    top_domains: Optional[List[Optional[TopDomainInfo]]] = Field(default=None, description=r"top domains citing the target keyword. contains objects with top domains citing the target category and citation count per each domain")
    ...
    connotation_types: Optional[Dict[str, Optional[StrictInt]]] = Field(default=None, description=r"connotation types. contains types of sentiments (sentiment polarity) related to the category citation and citation count per each sentiment type. possible connotation types: 'positive', 'negative', 'neutral'")
