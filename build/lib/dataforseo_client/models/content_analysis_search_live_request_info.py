...
class ContentAnalysisSearchLiveRequestInfo(BaseModel):
    ...
    search_mode: Optional[StrictStr] = Field(default=None, description=r"results grouping type. optional field. possible grouping types:. as_is – returns all citations for the target keyword. one_per_domain – returns one citation of the keyword per domain. default value: as_is")
    limit: Optional[StrictInt] = Field(default=None, description=r"the maximum number of returned citations. optional field. default value: 100. maximum value: 1000")
    ...
    order_by: Optional[List[Optional[StrictStr]]] = Field(default=None, description=r"results sorting rules. optional field. you can use the same values as in the filters array to sort the results. possible sorting types:. asc – results will be sorted in the ascending order. desc – results will be sorted in the descending order. you should use a comma to set up a sorting type. example:. ['content_info.sentiment_connotations.anger,desc']. default rule:. ['content_info.sentiment_connotations.anger,desc']. note that you can set no more than three sorting rules in a single request. you should use a comma to separate several sorting rules. example:. ['content_info.sentiment_connotations.anger,desc','keyword_data.keyword_info.cpc,desc']")
    offset: Optional[StrictInt] = Field(default=None, description=r"offset in the results array of returned citations. optional field. default value: 0. if you specify the 10 value, the first ten citations in the results array will be omitted and the data will be provided for the successive citations")
    offset_token: Optional[StrictStr] = Field(default=None, description=r"offset token for subsequent requests. optional field. provided in the identical field of the response to each request;. use this parameter to avoid timeouts while trying to obtain over 10,000 results in a single request;. by specifying the unique offset_token value from the response array, you will get the subsequent results of the initial task;. offset_token values are unique for each subsequent task. Note: if the offset_token is specified in the request, all other parameters except limit will not be taken into account when processing a task")
