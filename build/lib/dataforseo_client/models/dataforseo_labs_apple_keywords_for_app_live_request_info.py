...
class DataforseoLabsAppleKeywordsForAppLiveRequestInfo(BaseModel):
    ...
    order_by: Optional[List[Optional[StrictStr]]] = Field(default=None, description=r"results sorting rules. optional field. you can use the same values as in the filters array to sort the results;. possible sorting types:. asc – results will be sorted in the ascending order;. desc – results will be sorted in the descending order;. you should use a comma to specify a sorting type;. example:. ['ranked_serp_element.serp_item.rank_group,asc']. Note: you can set no more than three sorting rules in a single request;. you should use a comma to separate several sorting rules;. example:. ['ranked_serp_element.serp_item.rank_group,desc','keyword_data.keyword_info.search_volume,asc']. default rule:. ['keyword_data.keyword_info.search_volume,desc']. Note: if the item_types array contains item types that are different from organic, the results will be ordered by the first item type in the array")
