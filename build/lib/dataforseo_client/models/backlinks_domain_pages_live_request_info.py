...
class BacklinksDomainPagesLiveRequestInfo(BaseModel):
    ...
    """ # noqa: E501
    target: Optional[StrictStr] = Field(default=None, description=r"domain or subdomain. required field. a domain or a subdomain should be specified without https:// and www.. example:. forbes.com")
    ...
    backlinks_status_type: Optional[StrictStr] = Field(default=None, description=r"set what backlinks to return and count. optional field. you can use this field to choose what backlinks will be returned and used for aggregated metrics;. possible values:. all – all backlinks will be returned and counted;. live – backlinks found during the last check will be returned and counted;. lost – lost backlinks will be returned and counted;. default value: live")
    ...
    order_by: Optional[List[Optional[StrictStr]]] = Field(default=None, description=r"results sorting rules. optional field. you can use the same values as in the filters array to sort the results. possible sorting types:. asc – results will be sorted in the ascending order. desc – results will be sorted in the descending order. you should use a comma to set up a sorting type. example:. ['page_summary.backlinks,desc']. note that you can set no more than three sorting rules in a single request. you should use a comma to separate several sorting rules. example:. ['page_summary.backlinks,desc','page_summary.rank,asc']")
