...
class KeywordsDataGoogleAdsAdTrafficByKeywordsLiveResultInfo(BaseModel):
    ...
    impressions: Optional[StrictInt] = Field(default=None, description=r"projected number of ad impressions. number of impressions an ad is projected to get within the specified time period. if there is no data, then the value is null. learn more about impressions in this help center article")
    ctr: Optional[StrictFloat] = Field(default=None, description=r"projected click through rate (CTR) of the advertisement. number of clicks an ad is projected to receive divided by the number of ad impressions; the CTR is projected for the specified time period. if there is no data, then the value is null")
