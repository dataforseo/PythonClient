...
**search_volume** | **StrictInt** | <em>monthly average search volume rate</em><br>represents the (approximate) number of searches for the keyword on the Bing search engine, depending on the user’s targeting<p>search volume is rounded to the closest decimal values<p>if there is no data, then the value is_<code>null</code>n |[optional]|
